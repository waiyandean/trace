...
def render_sql(items, conversions, locations, suppliers, source):
    lines = [
...
def main():
    ...
    args.out.write_text(render_sql(items, conversions, locations, suppliers, args.workbook.name))
    ...
    summary = (
        ...
        f"{len(locations)} locations, {len(suppliers)} suppliers -> {args.out}"
    )
    text = report.render()
    print(summary)
    print()
